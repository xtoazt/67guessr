...
// import BannerText from "@/components/bannerText"; // Removed
...
// import MerchModal from "@/components/merchModal"; // Removed
...
export default function Home({ }) {
    ...
    useEffect(() => {
        // Only show CoolMath splash if explicitly requested via URL parameter
...
`)
    ...
    function crazyMidgame(adFinished = () => { }) {
        ...
        } else if (process.env.NEXT_PUBLIC_COOLMATH === "true" && inCoolMathGames && Date.now() - window.lastCoolmathAd > 120000) {
            try {
                window.lastCoolmathAd = Date.now();
                function onEnd() {
                    adFinished()
    ...
    return (
        <>
            ...
            <main className={`home`} id="main">
                ...
                {loading && (
                    <div className="text-center p-lg bg-gray-100 text-gray-800 rounded">
                        {text("loading")}...
                    </div>
                )}
